 ...
 * "threw". During the 2026-08-03 outage `fetchVideoSource()` returned `null`
 * and `fetchVideoTokenSilent()` logged a warning and returned `null` — nothing
 * threw and no `error` event was emitted, so to every error path in this
 * codebase a seven-hour failure looked like a series of successful calls
 * returning nothing. Call sites must therefore call {@link recordFailure} on
 * their empty-result branch, not only from `catch`.
 *
 * Self-healing is deliberate. That outage was resolved by power-cycling the
 * camera; a breaker that stayed open until the process restarted would have
 * kept the cameras dark long after they were healthy again.
 *
