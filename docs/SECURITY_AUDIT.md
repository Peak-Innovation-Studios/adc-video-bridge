...
- go2rtc is built from source ([HKSV#2130](https://github.com/AlexxIT/go2rtc/pull/2130) is unreleased), so the `alexxit/go2rtc` image digest is replaced rather than lost: toolchain by digest (`golang:1.24.13-alpine3.22@sha256:3641e0d9b931dc4f2f185dcd669c4679670e9277c8166a838ddb98a2d4389cb5`, `alpine:3.22@sha256:14358309a308569c32bdc37e2e0e9694be33a9d99e68afb0f5ff33cc1f695dce`), source by commit `2464e5670accb715d94fa9c7fb7cc1d2107b466b`. 🔴 **The source repository changed on 2026-08-25 from `skrashevich/go2rtc` to `Mo3he/go2rtc` — a DIFFERENT MAINTAINER, and therefore a supply-chain change rather than a pin bump.** The branch is the previous pin (`506cfa7`) plus 25 commits and zero behind. Verified before adoption: builds clean, `go vet` clean, HKSV and HAP test suites pass, and a package-level diff of the full suite against `506cfa7` showed **zero regressions and two packages fixed** (`pkg/hap/tlv8`, `pkg/hap/camera`), both of which fail on the commit it replaces. ⚠️ Not every one of the 25 commits is HKSV work: one is a merge from upstream `master` carrying unrelated changes. ✅ This removed `patches/go2rtc-hap-auth-exempt.patch`; the HAP auth exemption is now in the pinned source. Revert trigger: `docs/INVARIANTS.md`.
...
- A pre-commit secret scan (`scripts/scan-secrets.mjs`, wired via `.githooks/pre-commit`) **blocks** the commit on a finding rather than reporting it — RTSP credentials, HomeKit `pairings`/`device_private`, MACs, long hex values, camera ids, private LAN addresses against an allowlist of documented examples, and the mobile API's `Haiku` anchored on its key. 🔑 **Enable per clone with `git config core.hooksPath .githooks`; it is not automatic.** ⚠️ It scans **added lines in the staged diff**, not the working tree — the repository already contains real values in committed history, and a whole-tree scan would fail every commit and be disabled. ⚠️ Findings print the rule and file:line but **truncate the matched value**, because a scanner that echoes what it found leaks it into terminal scrollback and CI logs. 🔎 Verified by planting a canary of each rule and confirming a control of legitimate content passes; it caught a real LAN address committed as a test fixture the same day it was written.
