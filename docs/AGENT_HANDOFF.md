...
- **Last agent:** Codex
- **Updated:** 2026-08-02 (post-merge deployment documentation prepared)
- **Branch / HEAD:** `agent/document-synology-updates`, based on merged `main` at `5bab6d7`; its latest local commit contains the documentation update. Internal PR [Peak-Innovation-Studios/adc-video-bridge#1](https://github.com/Peak-Innovation-Studios/adc-video-bridge/pull/1) is merged.
- **Working tree:** Expected clean after this handoff. The documentation commit is local only and has not been pushed.
...
- **Whose turn:** David — authorize any documentation push/PR, then migrate and rebuild Kaikoura.

### What's left (priority order)

1. Push the local documentation commit and open an internal PR only if David authorizes it.
2. Migrate/rebuild Kaikoura from `main` and verify `front` remains on one RTSP publisher.
3. Confirm snapshots and live video in Apple Home, then keep the pilot stable before motion/HKSV.
